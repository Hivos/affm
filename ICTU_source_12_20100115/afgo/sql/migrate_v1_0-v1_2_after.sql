...
ALTER TABLE AFGO_T_RevenueExpense
DROP COLUMN FormalizedCommitmentAmt;

ALTER TABLE AFGO_T_RevenueExpense
DROP COLUMN PlannedCommitmentAmt;

UPDATE AFGO_QuotationResponse
SET IsPurchaseDomain=IsMantel;

UPDATE AD_ClientInfo ci
SET ci.AFGO_Calendar_ID=
(
	SELECT AD_Client_ID
	FROM AFGO_Calendar
	WHERE AD_Client_ID=ci.AD_Client_ID
	AND AD_Org_ID=0
	AND IsActive='Y'
)
WHERE ci.AFGO_Calendar_ID IS NULL
AND
(
	SELECT COUNT(*)
	FROM AFGO_Calendar
