 ...
 *
 *  @author Jorg Janke
 *  @version  $Id: MLookupFactory.java,v 1.1 2009/10/02 11:36:28 tomassen Exp $
 */
	...
	static public String getLookup_TableDirEmbed (Language language,
			String ColumnName, String BaseTable, String BaseColumn)
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				{
					String columnSQL = TableName + "." + ldc.ColumnName;
					if (!TableName.equals(TableNameTrl))
						columnSQL = "COALESCE(" + TableNameTrl + "." + ldc.ColumnName + "," + TableName + "." + ldc.ColumnName + ")";
					//jz EDB || problem
					if (DB.isPostgreSQL())
						embedSQL.append("COALESCE(TO_CHAR(" + columnSQL + ",'')");
					else
						embedSQL.append(columnSQL);
				}
			}

			embedSQL.append(" FROM ").append(TableName);
			if (!TableName.equals(TableNameTrl))
				embedSQL.append(" LEFT OUTER JOIN " + TableNameTrl + " ON (" + TableNameTrl + "." + ColumnName + "=" + TableName + "." + ColumnName + " AND " + TableNameTrl + ".AD_Language='" + language.getAD_Language() + "')");
			embedSQL.append(" WHERE ").append(BaseTable).append(".").append(BaseColumn);
