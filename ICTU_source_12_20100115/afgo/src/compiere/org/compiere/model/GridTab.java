 ...
 *  </pre>
 *  @author 	Jorg Janke
 *  @version 	$Id: GridTab.java,v 1.1 2009/06/02 08:58:20 tomassen Exp $
 */
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	public String getTrxInfo()
	{
		...
		{
			int Record_ID = m_vo.ctx.getContextAsInt( m_vo.WindowNo, "C_InvoiceBatch_ID");
		...
		else if (m_vo.TableName.startsWith("C_Order") || m_vo.TableName.startsWith("C_Invoice"))
		{
			...
			if (isOrder)
			{
				Record_ID = m_vo.ctx.getContextAsInt( m_vo.WindowNo, "C_Order_ID");
			...
			{
				Record_ID = m_vo.ctx.getContextAsInt( m_vo.WindowNo, "C_Invoice_ID");
			...
			try
			{
				...
				mfMC = new MessageFormat(Msg.getMsg(Env.getAD_Language(m_vo.ctx), "OrderSummaryMC"));
			}
			catch (Exception e)
			{
			...
			try
			{
				...
				if (rs.next())
				{
					...
					arguments[5] = m_vo.ctx.getContext("$CurrencyISO");
					filled = true;
				}
				rs.close();
				pstmt.close();
		...
		{
			int Record_ID = m_vo.ctx.getContextAsInt( m_vo.WindowNo, "S_TimeExpense_ID");
			log.fine(m_vo.TableName + " - " + Record_ID);
			MessageFormat mf = null;
			try
			{
