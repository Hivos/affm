 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: VDocAction.java,v 1.2 2009/08/18 14:12:38 tomassen Exp $
 */
	...
	implements ActionListener
{
	...
	private void dynInit(int Record_ID)
	{
		 ...
		 * Purchase Commitment
		 */
		else if (m_AD_Table_ID == MAFGOPurchaseCommitment.Table_ID)
		{
			if (DocStatus.equals(DocActionConstants.STATUS_Invalid))
			{
				for (int i = 0; i < options.length; i++)
				{
					if (DocActionConstants.ACTION_Complete.equals(options[i]))
						options[i] = DocActionConstants.ACTION_Prepare;
				}
			}
		}
		
		/**
		 * disable void & reverse actions for all completed documents
		 * 10002556
		 */
		if (DocStatus.equals(DocActionConstants.STATUS_Completed))
		{
    		...
    		for (int i = 0; i < options.length; i++)
    		{
    			if (options[i] == null)
    				continue;
    			if (DocActionConstants.ACTION_Reverse_Accrual.equals(options[i]))
    				continue;
    			if (DocActionConstants.ACTION_Reverse_Correct.equals(options[i]))
    				continue;
    			if (DocActionConstants.ACTION_Void.equals(options[i]))
    				continue;
