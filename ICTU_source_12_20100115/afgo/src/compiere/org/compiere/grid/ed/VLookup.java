 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: VLookup.java,v 1.2 2010/01/04 13:30:19 tomassen Exp $
 */
	...
	implements VEditor, ActionListener, FocusListener
{
		...
		Lookup lookup)
	{
		...
		if (columnName.equals("C_BPartner_ID"))
		{
			// ActFact
			// 3529
			// Only show quick create & update entries when role has access to BPartner window
			if (Boolean.TRUE.equals(MRole.getDefault().getWindowAccess(MTable.get(Env.getCtx(),MBPartner.Table_Name).getAD_Window_ID())))
			{
