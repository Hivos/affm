 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: DataEngine.java,v 1.2 2009/12/16 15:42:13 tomassen Exp $
 */
public class DataEngine
{
		...
		String reportName, String tableName)
	{
		...
		for (int element : orderAD_Column_IDs) {
			log.finest("Order AD_Column_ID=" + element);
		...
		try
		{
			...
			while (rs.next())
			{
					...
					)
				{
					...
					{
						/*
						 * ActFact: JOIN _Trl Table
						 */
						//System.err.println("TableName=" + tr.TableName + ", synonym=" + m_synonym + ", DisplayColumn=" + display);
						//	=> A.Name AS AName, Table.ID,
						if (tr.IsValueDisplayed)
							sqlSELECT.append(m_synonym).append(".Value||'-'||");
						if (tr.IsTranslated && !Env.isBaseLanguage(m_language.getAD_Language(), tr.TableName))
							sqlSELECT.append(m_synonym + "TRL").append(".").append(display);
						else
						...
						if (tr.IsTranslated && !Env.isBaseLanguage(m_language.getAD_Language(), tr.TableName))
							...
							sqlGROUP.append(m_synonym + "TRL").append(".").append(display).append(",");
						sqlGROUP.append(tableName).append(".").append(ColumnName).append(",");
						//System.err.println("GROUP=" + sqlGROUP.toString());
						orderName = m_synonym + display;
						...
						if (tr.IsTranslated && !Env.isBaseLanguage(m_language.getAD_Language(), tr.TableName))
							sqlFROM.append("LEFT OUTER JOIN " + tr.TableName + "_Trl " + m_synonym + "TRL ON (" + tableName + "." + ColumnName + "=" + m_synonym + "TRL." + tr.KeyColumn + " AND " + m_synonym + "TRL.AD_Language='" + m_language.getAD_Language() + "')");
	...
	public static TableReference getTableReference (int AD_Reference_Value_ID, Language language)
	{
		...
		{
			System.err.println("TRL: AD_Reference_ID=" + AD_Reference_Value_ID + ", " + tr.TableName + "->" + tr.TableName + "_Trl");
