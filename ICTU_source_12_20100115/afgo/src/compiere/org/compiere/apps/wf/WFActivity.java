 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: WFActivity.java,v 1.5 2009/09/14 09:47:40 tomassen Exp $
 */
	...
	implements FormPanel, ActionListener, ListSelectionListener
{
	...
	public void display(int index)
	{
		...
		if (X_AD_WF_Node.ACTION_UserChoice.equals(node.getAction()))
		{
			...
			if (m_column != null && m_column.get_ID() != 0)
			{
				...
				{
					// ActFact 2839
					// translate yes/no combo
					//ValueNamePair[] values = MRefList.getList(319, false);		//	_YesNo
					ValueNamePair[] values = getList(m_activity.getCtx(), 319, false);		//	_YesNo
					// ActFact 2839
					
					// ActFact 2837
					ArrayList<ValueNamePair> valueList = new ArrayList<ValueNamePair>();
					for (int i = 0; i < values.length; i++)
					{
						ValueNamePair entry = values[i];
						if ("Y".equals(entry.getValue()))
						{
							valueList.add(0, entry);
						}
						else
							valueList.add(entry);
					}
				...
				{
					// ActFact 2839
					// translate ref list combo
					//ValueNamePair[] values = MRefList.getList(m_column.getAD_Reference_Value_ID(), false);
					ValueNamePair[] values = getList(m_activity.getCtx(), m_column.getAD_Reference_Value_ID(), false);
					// ActFact 2839
	...
	private void cmd_zoom()
	{
		...
		if (m_activity == null)
			return;
		// ActFact 2868 (use window specified on node instead of standard zoom window)
		MWFNode node = m_activity.getNode();
		if (node.getAD_Window_ID() > 0)
		{
    		AWindow frame = new AWindow();
    		if (!frame.initWindow(node.getAD_Window_ID(), Query.getEqualQuery(MTable.getTableName(m_activity.getCtx(), m_activity.getAD_Table_ID()) + "_ID", m_activity.getRecord_ID())))
    			return;
	...
	// ActFact 2839 
	// translate yes/no combo
	private static CLogger s_log = CLogger.getCLogger(WFActivity.class);
	/**
	 * 	Get Reference List
